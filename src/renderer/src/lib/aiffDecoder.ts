...
export function decodeAiff(bytes: Uint8Array): AiffPcm {
  ...
  if (readTag(view, 0) !== 'FORM') throw new Error('AIFF 파일이 아님 (FORM 청크 없음)')
  const formType = readTag(view, 8)
  if (formType !== 'AIFF' && formType !== 'AIFC') throw new Error(`지원하지 않는 AIFF 형식: ${formType}`)
  ...
  if (ssndDataStart < 0) throw new Error('AIFF 파일에 SSND(샘플 데이터) 청크가 없음')
  if (numChannels <= 0 || numFrames <= 0) throw new Error('AIFF 파일의 COMM 청크가 올바르지 않음')
  ...
  for (let frame = 0; frame < numFrames; frame++) {
    for (let ch = 0; ch < numChannels; ch++) {
      ...
      if (isFloat && bitsPerSample === 32) {
        ...
        let v = littleEndian ? (b2 << 16) | (b1 << 8) | b0 : (b0 << 16) | (b1 << 8) | b2
