...
export default function AnalysisPanel({ playerRef, track }: AnalysisPanelProps): JSX.Element {
  const [open, setOpen] = useState(() => loadBool('soundlib.analysis.open', true))
  const [peakOn, setPeakOn] = useState(() => loadBool('soundlib.analysis.peak', true))
  const [widthOn, setWidthOn] = useState(() => loadBool('soundlib.analysis.width', true))
  ...
  useEffect(() => {
    ...
    ;[-90, -45, 0, 45, 90].forEach((deg) => {
      ...
      ctx.strokeStyle = deg === 0 ? 'rgba(255,255,255,0.35)' : 'rgba(255,255,255,0.16)'
      ctx.stroke()
    })
  ...
  useEffect(() => {
    ...
    function accentColor(alpha: number): string {
      ...
      return v ? `color-mix(in srgb, ${v} ${Math.round(alpha * 100)}%, transparent)` : `rgba(163,227,193,${alpha})`
    }
    ...
    function applyCorrelation(corr: number): void {
      ...
      if (corrValRef.current) corrValRef.current.textContent = isFinite(corr) ? corr.toFixed(2) : '—'
    }
    function renderIdle(): void {
      if (peakOn) {
        ...
        setText(valLRef.current, fmtDb(isFinite(meterState.current.holdL) ? meterState.current.holdL : null))
        setText(valRRef.current, fmtDb(isFinite(meterState.current.holdR) ? meterState.current.holdR : null))
      }
      if (widthOn) {
        fadeGonio()
        applyCorrelation(1)
    ...
    function tick(ts: number): void {
      ...
      if (peakOn) {
        ...
        st.peakL = dbL > st.peakL ? dbL : Math.max(DISPLAY_FLOOR_DB, st.peakL - decay)
        st.peakR = dbR > st.peakR ? dbR : Math.max(DISPLAY_FLOOR_DB, st.peakR - decay)
        ...
        barLRef.current?.classList.toggle('analysis__hbar-fill--clip', dbL > -1)
        barRRef.current?.classList.toggle('analysis__hbar-fill--clip', dbR > -1)
      ...
      if (widthOn) {
        if (!tap.isPlaying) {
          ...
          const instantWidth = widthDenom > 1e-9 ? clampPct((sideRms / widthDenom) * 100) : 0
          ...
          st.correlation += (instantCorr - st.correlation) * CORRELATION_SMOOTHING

          drawGonio(bufL, bufR)
          applyCorrelation(st.correlation)
          setText(widthValRef.current, tap.isMono ? 'Mono' : `${Math.round(st.widthPct)}%`)
        }
      }
    }
  ...
  function toggle(key: string, current: boolean, setter: (v: boolean) => void): void {
    const next = !current
    setter(next)
    saveBool(key, next)
  }
      ...
      <div className="analysis__header">
        ...
        <div className="analysis__toggles">
          ...
          <button
            ...
            onClick={() => toggle('soundlib.analysis.width', widthOn, setWidthOn)}
          ...
          {track && peakOn && (
            <div className="analysis__group">
              <div className="analysis__group-label-row">
                ...
                <button className="analysis__reset-btn" onClick={resetPeakHold} title="Peak Hold 초기화">
                  Reset
                </button>
              </div>
          ...
          {track && widthOn && (
            <div className="analysis__group">
              ...
              <div className="analysis__gonio">
                <div className="analysis__gonio-canvases" style={{ aspectRatio: `${GONIO_W} / ${GONIO_H}` }}>
                  <canvas className="analysis__gonio-canvas" ref={gonioBgCanvasRef} width={GONIO_W} height={GONIO_H} />
                  <canvas className="analysis__gonio-canvas" ref={gonioFgCanvasRef} width={GONIO_W} height={GONIO_H} />
                </div>
