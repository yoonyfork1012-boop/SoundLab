...
export default function IndexingIndicator({ progress }: IndexingIndicatorProps): JSX.Element {
  const total = progress?.total ?? 0
  const scanned = progress?.scanned ?? 0
  const indeterminate = !progress || progress.phase === 'discovering' || total <= 0
    ...
    <div className="indexing">
      ...
      <div className="indexing__text">
        ...
        <span className="indexing__count">
          {indeterminate ? '파일 검색 중…' : `${scanned.toLocaleString()} / ${total.toLocaleString()}`}
        </span>
      </div>
    </div>
  )
