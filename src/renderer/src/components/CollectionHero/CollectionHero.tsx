...
function MosaicCover({ tracks }: { tracks: Track[] }): JSX.Element {
  const quads = [0, 1, 2, 3].map((i) =>
    tracks[i] ? colorForCategory(tracks[i].category) : null,
  );
  return (
            ...
            <div className="coll-hero__bar">
              {breakdown.map((b) => (
                <div
                  ...
                  style={{
                    width: `${(b.count / tracks.length) * 100}%`,
                    background: b.color,
                  }}
            ...
            <div className="coll-hero__legend">
              {shown.map((b) => (
                <span className="coll-hero__legend-item" key={b.category}>
                  <span
                    className="coll-hero__legend-dot"
              ...
              {restCount > 0 && (
                <span className="coll-hero__legend-item coll-hero__legend-item--rest">
                  +{restCount} more
                </span>
              )}
            </div>
