...
function relSegments(filePath: string, rootPath: string | null): string[] | null {
...
function matchesSegment(pattern: string, value: string): boolean {
  ...
  const source = '^' + escapeRegex(pattern).replaceAll('\\*', '.*').replaceAll('\\?', '.') + '$'
  const regex = new RegExp(source, 'i')
  return regex.test(value)
}

function matchesPrefix(pathSegs: string[], patternSegs: string[]): boolean {
  if (patternSegs.length === 0 || pathSegs.length < patternSegs.length) return false
...
function customPublisher(filePath: string, rootPath: string | null, customPath: string | null): string | null {
  ...
  if (rootRel && matchesPrefix(rootRel, patternSegs)) return lastConcreteSegment(patternSegs)
  const fileSegs = fileDirSegments(filePath)
  if (matchesPrefix(fileSegs, patternSegs)) return lastConcreteSegment(patternSegs)
  return null
}
...
export function resolvePublisherFromPath(
  ...
  if (rule.mode === 'custom') return customPublisher(filePath, libraryRootPath, rule.customPath)
...
export function resolveTrackPublisher(
  ...
  return resolvePublisherFromPath(track.filePath, library?.rootPath ?? null, rule)
}
