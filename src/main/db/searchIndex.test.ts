...
describe("검색 인덱스 (FTS5 trigram)", () => {
  ...
  it("이미 트랙이 있는 DB를 열면 기존 행까지 색인한다 (업그레이드 경로)", async () => {
    ...
    // 제안은 낱말 단위라 "LegacyOrphan.wav"는 legacy / orphan으로 쪼개진다.
    expect(q.suggestTerms("legac")).toContain("legacy");
    expect(q.suggestTerms("rebui")).toContain("rebuild");
  });
});
...
describe("검색 자동완성 (fts5vocab 사전)", () => {
  ...
  it("가장 흔한 완성형이 맨 앞이다", () => {
    const out = q.suggestTerms("w");
    // 목록의 주인공은 조합 제안이라 변형(wood/water/wonce)은 뒤에 몇 개만 붙는다.
    // 남더라도 흔한 완성형(whoosh 5회)보다 앞일 수는 없다.
    expect(out[0]).toBe("whoosh");
    const rare = out.indexOf("wonce");
    if (rare >= 0) expect(rare).toBeGreaterThan(0);
  });

  it("완성된 낱말에는 조합을 제안한다 (Soundly식 'whoosh fast')", () => {
    // 같은 낱말이 여러 폴더에 걸쳐 나와야 그 팩만의 표기가 아니라고 본다.
    for (const dir of ["packA", "packB", "packC"]) {
      addTrack({
        filePath: `/lib/${dir}/metal-fast.wav`,
        filename: "MetalFastHit.wav",
      });
    }
    const out = q.suggestTerms("metal");
    expect(out[0]).toBe("metal");
    expect(out).toContain("metal fast");
  });

  it("한 폴더에서만 쓰는 꼬리표는 조합으로 제안하지 않는다", () => {
    for (let i = 0; i < 5; i++) {
      addTrack({
        filePath: `/lib/onepack/glass-${i}.wav`,
        filename: `GlassBreak_ACME_${i}.wav`,
      });
    }
    // acme는 한 폴더에만 있으므로(제작사 꼬리표) 조합에서 빠진다.
    expect(q.suggestTerms("glass")).not.toContain("glass acme");
  });
