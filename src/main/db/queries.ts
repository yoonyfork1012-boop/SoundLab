...
// 제안을 "더 좁히는 낱말"로 만들기 위한 표본 상한. "whoosh"처럼 흔한 낱말은 2만 건이
// 걸리는데, 동반 낱말 순위는 몇 천 건만 봐도 같은 결과가 나온다. 자동완성은 타이핑 중에
// 매번 도는 질의라 이 상한이 응답 시간을 지배한다.
const COOCCUR_SAMPLE = 4000;
...
// 제안 낱말을 뽑을 원문. 파일명이 주지만, 카테고리·태그로만 붙여둔 말(예: 직접 넣은
// "cinematic" 태그)도 같은 자격으로 제안돼야 한다. 대소문자를 살려서 가져와야
// splitWords가 camelCase를 쪼갤 수 있으므로 lower()를 씌우지 않는다.
const SUGGEST_TEXT_EXPR = `t.filename || ' ' || COALESCE(t.category, '') || ' ' ||
    COALESCE(t.subcategory, '') || ' ' || COALESCE(t.description, '') || ' ' ||
    replace(replace(replace(COALESCE(t.tags, ''), '[', ' '), ']', ' '), '"', ' ')`;

/**
 * 파일명을 낱말로 쪼갠다.
 *
 * unicode61 토크나이저는 구분자만 보므로 "WHOOSHMetallicThin.wav"가 통째로 한 낱말이
 * 된다(제안에 whooshmetallicth 같은 것이 뜨던 이유다). 여기서는 대소문자 경계까지
 * 끊어 "whoosh / metallic / thin"으로 만든다.
 */
 ...
 * 제안 낱말로 쓸 만한가.
 *
 * SFX 라이브러리의 파일명은 절반이 약어다 — 제작사 태그(B00M, ESM), UCS CatID 꼬리표
 * (WHSH, DSGN, CRWD), 채널 표기. 이것들이 특정 팩 안에서 압도적으로 흔해서 그냥 빈도만
 * 세면 제안이 통째로 약어로 채워진다. 영어 낱말에는 모음이 있다는 점 하나로 대부분
 * 걸러진다(whsh·dsgn·crwd는 나가고 fast·metal·short는 남는다).
 */
function isUsefulWord(w: string): boolean {
  return (
    w.length >= MIN_TERM_LENGTH &&
    !/\d/.test(w) &&
    /[aeiouy]/.test(w) &&
    !EXCLUDED_TERMS.has(w) &&
    !COOCCUR_STOPWORDS.has(w)
  );
}

/** gun→gunshot, footstep→footsteps, door→outdoor 처럼 겹치는 낱말은 조합이 되지 못한다. */
function overlaps(head: string, w: string): boolean {
  return w.includes(head) || head.includes(w);
}

// 자동완성 질의는 statement 캐시에 남기지 않는다(prep이 아니라 매번 prepare).
// 캐시에 남으면 tracks_fts를 참조하는 statement가 살아 있어, 색인을 다시 만들 때
// (DROP TABLE tracks_fts) 가상 테이블이 잠겨 재구축이 조용히 건너뛰어질 수 있다.
function once(sql: string): Database.Statement {
  return getDb().prepare(sql);
}

/**
 * 접두어로 시작하는 실제 낱말을 빈도순으로.
 *
 * 사전(tracks_terms)에서 바로 뽑으면 안 된다 — unicode61 토크나이저라 "MetalFastHit.wav"가
 * metalfasthit 한 낱말로 들어가고, 소문자로 저장돼 있어 나중에 쪼갤 단서도 없다. 그래서
 * 대소문자가 살아 있는 원문을 표본으로 훑어 낱말을 만든다. 사전은 3글자 미만 입력의
 * 폴백으로만 남는다 — trigram 인덱스가 3글자부터라 그 아래는 MATCH로 표본을 못 모은다.
 */
function prefixWords(p: string, limit: number): string[] {
  if (p.length < FTS_MIN_QUERY_LENGTH) {
    const rows = once(
  ...
  const names = once(
    `SELECT ${SUGGEST_TEXT_EXPR} FROM tracks_fts f JOIN tracks t ON t.id = f.rowid
     WHERE f.blob MATCH ? LIMIT ?`,
  )
    .pluck()
    .all(ftsPhrase(p), COOCCUR_SAMPLE) as string[];
  const freq = new Map<string, number>();
  for (const name of names) {
    for (const w of new Set(splitWords(name))) {
      if (!w.startsWith(p) || w.length < MIN_TERM_LENGTH) continue;
      if (EXCLUDED_TERMS.has(w)) continue;
      freq.set(w, (freq.get(w) ?? 0) + 1);
    }
  }
  // 같은 빈도면 짧은 쪽이 앞 — "whoosh"가 "whooshes"보다 고를 만하다.
  return [...freq.entries()]
...
/**
 * 그 낱말이 든 파일들에서 함께 자주 쓰이는 낱말 — "whoosh" → fast / short / long.
 *
 * Soundly의 제안이 이 모양이다: 낱말 하나를 치면 그 낱말을 더 좁히는 조합을 보여준다.
 * 색인 낱말을 접두어로만 훑으면(예전 방식) whooshes·whooshbang 같은 변형만 나와서
 * "뭘 더 칠지"에 도움이 안 됐다.
 */
function cooccurringWords(word: string, limit: number): string[] {
  const rows = once(
    `SELECT ${SUGGEST_TEXT_EXPR}, t.file_path FROM tracks_fts f JOIN tracks t ON t.id = f.rowid
     WHERE f.blob MATCH ? LIMIT ?`,
  )
    .raw()
    .all(ftsPhrase(word), COOCCUR_SAMPLE) as [string, string][];
  const files = new Map<string, number>();
  const dirs = new Map<string, Set<string>>();
  for (const [name, path] of rows) {
    const cut = Math.max(path.lastIndexOf("\\"), path.lastIndexOf("/"));
    const dir = cut > 0 ? path.slice(0, cut) : path;
    // 한 파일 안에서 같은 낱말이 여러 번 나와도 한 번만 센다(파일 수 기준 빈도).
    for (const w of new Set(splitWords(name))) {
      if (overlaps(word, w) || !isUsefulWord(w)) continue;
      files.set(w, (files.get(w) ?? 0) + 1);
      let seen = dirs.get(w);
      if (!seen) dirs.set(w, (seen = new Set()));
      seen.add(dir);
    }
  }
  // 한 폴더에서만 쓰이는 낱말은 그 팩의 내부 표기(제작사 약어·세션 이름)일 뿐이라 버린다.
  // 여러 팩에 걸쳐 나오는 낱말이라야 "이 라이브러리에서 통하는 말"이다. 그래서 순위도
  // 파일 수가 아니라 폴더 수를 먼저 본다 — 한 팩이 같은 낱말로 1,000개를 쏟아내도
  // 여러 팩에 고루 나오는 낱말을 이기지 못한다.
  return [...files.entries()]
    .map(([w, n]) => ({ w, n, d: dirs.get(w)?.size ?? 0 }))
    .filter((x) => x.d > 1)
    .sort((a, b) => b.d - a.d || b.n - a.n || a.w.localeCompare(b.w))
    .slice(0, limit)
    .map((x) => x.w);
}

/**
 * 검색창 자동완성. 마지막 낱말을 완성하거나, 완성된 낱말이면 조합을 제안한다.
 *
 *   "who"    → whoosh, whoosh fast, whoosh short, …
 *   "whoosh" → whoosh, whoosh fast, whoosh short, …
 */
export function suggestTerms(prefix: string, limit = SUGGEST_LIMIT): string[] {
  ...
  try {
    const words = prefixWords(p, limit);
    // 가장 흔한 완성형을 기준 낱말로 삼는다. 입력이 이미 완성된 낱말이면 그것이 1순위다.
    const head = words[0];
    if (!head) return [];
    // 조합이 목록의 주가 되도록 낱말 변형은 몇 개만 남긴다(whoosh 다음 whooshes 정도).
    const variants = words.slice(1, 3);
    const phrases = cooccurringWords(head, limit).map((w) => `${head} ${w}`);
    const out: string[] = [head];
    for (const item of [...phrases, ...variants]) {
