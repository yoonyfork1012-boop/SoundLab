...
// gif도 받는다 — 제작사 사이트가 팩 커버를 gif로 올려두는 경우가 있다(Articulated Sounds).
...
export function findCoverInDir(dir: string): string | null {
  ...
  for (const base of COVER_BASENAMES) {
    const hit = images.find(
      (e) => e.name.toLowerCase().replace(/\.[^.]+$/, "") === base,
    );
...
// 트랙 삭제(unlink) 시 해당 파일 경로로 캐시된 임베디드 아트워크를 정리한다.
// 캐시 키가 파일 경로 해시라서 삭제된 파일 것만 정확히 지울 수 있다(폴더 커버는 여러 파일이
// 공유하므로 개별 트랙 삭제로는 건드리지 않는다).
export function removeEmbeddedArtworkCache(filePath: string): void {
...
export async function getEmbeddedArtworkDataUrl(
  ...
  parseFile: (
    p: string,
  ) => Promise<{ common?: { picture?: Array<{ data: Uint8Array }> } }>,
): Promise<string | null> {
  const cache = join(ARTWORK_DIR, `emb_${sha1(filePath)}.jpg`);
  try {
    if (existsSync(cache)) return toDataUrl(readFileSync(cache));
